...
if 'env' not in st.session_state:
    ...
    st.session_state.last_clicked = None  # NEW — track which button was clicked
...
for i in range(9):
    with cols[i % 3]:
        ...
        if st.button(button_text, key=f"btn_{i}"):
            st.session_state.last_clicked = i  # Save which button was clicked

# If a button was clicked, handle the move
if st.session_state.last_clicked is not None:
    handle_move(st.session_state.last_clicked)
    st.session_state.last_clicked = None  # Reset after handling
